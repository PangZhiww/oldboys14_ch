...
#
# class file:
#     def __init__(self,name,sex,phone,job):    # 构造方法
#         self.name = name
#         self.sex = sex
#         self.phone = phone
#         self.job = job
...
# Person('alex',100,'男','oldwomen')
# ret = Person('alex',100,'男','oldwomen')    # 类名 + () 的这个过程: 实例化的过程(创建一个对象的过程)
#   Person() 实例化对象, 实例, 对象


# 1. 类名+() 产生一个实例(对象, 对象空间)
# 2. 自动执行类中的__init__方法, 将对象空间创给__init__中的self参数
# 3. 给对象封装相应的属性
# print(ret.__dict__)
...
class Person:
    ...
    def shop(self):
        print("%s都会消费..." % self.name)


bigsum = Person('大阳哥', '39', '男', '非男')
idea_ning = Person('印度阿宁', '19', '男', '女')
bigsum.work()
idea_ning.shop()
